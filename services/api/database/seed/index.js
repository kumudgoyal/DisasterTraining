...
async function main() {
    ...
    const orgs = [
        { name: '[SEED] NDMA Headquarters', type: client_1.OrgType.GOVERNMENT },
        { name: '[SEED] Maharashtra SDMA', type: client_1.OrgType.GOVERNMENT },
        { name: '[SEED] National Institute of Disaster Management', type: client_1.OrgType.GOVERNMENT },
        { name: '[SEED] Gujarat SDMA', type: client_1.OrgType.GOVERNMENT },
        { name: '[SEED] Tata Trust Disaster Program', type: client_1.OrgType.NGO },
        { name: '[SEED] Red Cross India', type: client_1.OrgType.NGO },
        { name: '[SEED] Bihar SDMA', type: client_1.OrgType.GOVERNMENT },
        { name: '[SEED] National Disaster Response Force', type: client_1.OrgType.GOVERNMENT },
    ];
    const createdOrgs = [];
    for (const o of orgs) {
        createdOrgs.push(await prisma.organization.create({ data: o }));
    }
    const findOrg = (name) => createdOrgs.find(o => o.name === name);
    console.log('8. Creating Users...');
    const users = [
        { name: 'Admin', email: 'admin@disaster-training.gov.in', role: client_1.Role.SUPER_ADMIN, orgId: findOrg('[SEED] NDMA Headquarters')?.id },
        { name: 'NDMA Admin', email: 'ndma@disaster-training.gov.in', role: client_1.Role.NDMA_ADMIN, orgId: findOrg('[SEED] NDMA Headquarters')?.id },
        { name: 'SDMA User', email: 'sdma.mh@gov.in', role: client_1.Role.SDMA, orgId: findOrg('[SEED] Maharashtra SDMA')?.id },
        { name: 'ATI User', email: 'ati@gov.in', role: client_1.Role.ATI, orgId: findOrg('[SEED] National Institute of Disaster Management')?.id },
        { name: 'NGO User', email: 'ngo@redcross.in', role: client_1.Role.NGO, orgId: findOrg('[SEED] Red Cross India')?.id },
        { name: 'Trainer', email: 'trainer@disaster-training.gov.in', role: client_1.Role.TRAINER, orgId: findOrg('[SEED] NDMA Headquarters')?.id },
        { name: 'Reviewer', email: 'reviewer@disaster-training.gov.in', role: client_1.Role.REVIEWER, orgId: findOrg('[SEED] NDMA Headquarters')?.id },
        { name: 'Data Entry', email: 'dataentry@disaster-training.gov.in', role: client_1.Role.DATA_ENTRY_OPERATOR, orgId: findOrg('[SEED] NDMA Headquarters')?.id },
        { name: 'Viewer', email: 'viewer@disaster-training.gov.in', role: client_1.Role.NDMA_VIEWER, orgId: findOrg('[SEED] NDMA Headquarters')?.id }
    ];
    const createdUsers = [];
    ...
    const trainingsData = [
        ...
        },
        {
            ...
            status: client_1.TrainingStatus.APPROVED,
            city: 'New Delhi',
        ...
        },
        {
            ...
            status: client_1.TrainingStatus.SUBMITTED,
            city: 'Patna',
    ...
    for (const t of trainingsData) {
        ...
        const training = await prisma.training.create({
            data: {
                ...
                reviewedBy: t.status === client_1.TrainingStatus.APPROVED || t.status === client_1.TrainingStatus.COMPLETED ? reviewer.id : null,
                startDate: new Date(),
    ...
    const completedTraining = createdTrainings.find(t => t.status === client_1.TrainingStatus.COMPLETED);
    const inProgressTraining = createdTrainings.find(t => t.status === client_1.TrainingStatus.IN_PROGRESS);
        ...
        for (let i = 1; i <= 10; i++) {
            ...
            await prisma.attendance.create({
                data: {
                    ...
                    participantId: participant.id,
                    attendanceDate: new Date(),
                    status: i % 3 === 0 ? client_1.AttendanceStatus.ABSENT : client_1.AttendanceStatus.PRESENT,
                    markedBy: creator.id
                }
            });
    ...
    const alertRules = [
        { name: 'LOW_ATTENDANCE', ruleType: 'ATTENDANCE', conditions: { threshold: 50 }, severity: client_1.Severity.HIGH },
        { name: 'COVERAGE_GAP', ruleType: 'COVERAGE', conditions: { type: 'gap' }, severity: client_1.Severity.MEDIUM },
        { name: 'MISSED_DEADLINE', ruleType: 'SCHEDULE', conditions: { daysOverdue: 2 }, severity: client_1.Severity.CRITICAL },
        { name: 'PENDING_APPROVAL', ruleType: 'WORKFLOW', conditions: { daysPending: 5 }, severity: client_1.Severity.LOW }
    ];
    const createdRules = [];
    ...
    const adminUser = createdUsers.find(u => u.role === client_1.Role.SUPER_ADMIN);
    if (adminUser) {
